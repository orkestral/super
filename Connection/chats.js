...
const make_mutex_1 = __importDefault(require("../Settings/make-mutex"));
...
const makeChatsSocket = (config) => {
    ...
    const interactiveQuery = async (userNodes, queryNode) => {
        ...
        const usyncNode = Internal_1.getBinaryNodeChild(result, 'usync');
        const listNode = Internal_1.getBinaryNodeChild(usyncNode, 'list');
        const users = Internal_1.getBinaryNodeChildren(listNode, 'user');
        return users;
    };
    const onWhatsApp = async (...jids) => {
        ...
        return results.map(user => {
            const contact = Internal_1.getBinaryNodeChild(user, 'contact');
    ...
    const fetchStatus = async (jid) => {
        ...
        if (result) {
            const status = Internal_1.getBinaryNodeChild(result, 'status');
            return {
    ...
    const updateProfilePicture = async (jid, content) => {
        const { img } = await Settings_1.generateProfilePicture(content);
        await query({
            tag: 'iq',
            attrs: {
                to: Internal_1.jidNormalizedUser(jid),
                type: 'set',
    ...
    const getBusinessProfile = async (jid) => {
        ...
        const profiles = Internal_1.getBinaryNodeChild(Internal_1.getBinaryNodeChild(results, 'business_profile'), 'profile');
        if (!profiles) {
            // if not bussines
        ...
        const address = Internal_1.getBinaryNodeChild(profiles, 'address');
        const description = Internal_1.getBinaryNodeChild(profiles, 'description');
        const website = Internal_1.getBinaryNodeChild(profiles, 'website');
        const email = Internal_1.getBinaryNodeChild(profiles, 'email');
        const category = Internal_1.getBinaryNodeChild(Internal_1.getBinaryNodeChild(profiles, 'categories'), 'category');
        const business_hours = Internal_1.getBinaryNodeChild(profiles, 'business_hours');
        const business_hours_config = business_hours && Internal_1.getBinaryNodeChildren(business_hours, 'business_hours_config');
        return {
            ...
            while (collectionsToHandle.size) {
                ...
                for (const name of collectionsToHandle) {
                    ...
                    if (state) {
                        ...
                        state = Settings_1.newLTHashState();
                    }
                    states[name] = state;
                    nodes.push({
                        tag: 'collection',
                ...
                const decoded = await Settings_1.extractSyncdPatches(result); // extract from binary node
                for (const key in decoded) {
                    const name = key;
                    const { patches, hasMorePatches, snapshot } = decoded[name];
                    try {
                        if (snapshot) {
                            const { state: newState, mutations } = await Settings_1.decodeSyncdSnapshot(name, snapshot, getAppStateSyncKey, initialVersionMap[name]);
                        ...
                        if (patches.length) {
                            const { newMutations, state: newState } = await Settings_1.decodePatches(name, patches, states[name], getAppStateSyncKey, initialVersionMap[name]);
                            await authState.keys.set({ 'app-state-sync-version': { [name]: newState } });
    ...
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs) => {
        ...
        const child = Internal_1.getBinaryNodeChild(result, 'picture');
        ...
        for (const { syncAction: { value: action }, index: [_, id, msgId, fromMe] } of actions) {
            ...
            if (action === null || action === void 0 ? void 0 : action.muteAction) {
                update.mute = ((_a = action.muteAction) === null || _a === void 0 ? void 0 : _a.muted) ?
                    Settings_1.toNumber(action.muteAction.muteEndTimestamp) :
                    undefined;
            }
                ...
                update.pin = ((_e = action.pinAction) === null || _e === void 0 ? void 0 : _e.pinned) ? Settings_1.toNumber(action.timestamp) : undefined;
            }
            else {
                //
            }
    ...
    const appPatch = async (patchCreate) => {
        ...
        await mutationMutex.mutex(async () => {
            ...
            const { patch, state } = await Settings_1.encodeSyncdPatch(patchCreate, myAppStateKeyId, initial, getAppStateSyncKey);
            const node = {
                tag: 'iq',
                attrs: {
                    to: Internal_1.S_WHATSAPP_NET,
            ...
            if (config.emitOwnEvents) {
                const result = await Settings_1.decodePatches(name, [{ ...patch, version: { version: state.version }, }], initial, getAppStateSyncKey);
    ...
    const fetchAbt = async () => {
        ...
        const propsNode = Internal_1.getBinaryNodeChild(abtNode, 'props');
        let props = {};
        if (propsNode) {
            props = Internal_1.reduceBinaryNodeToDictionary(propsNode, 'prop');
        }
    ...
    const fetchProps = async () => {
        ...
        const propsNode = Internal_1.getBinaryNodeChild(resultNode, 'props');
        let props = {};
        if (propsNode) {
            props = Internal_1.reduceBinaryNodeToDictionary(propsNode, 'prop');
        }
    ...
    */
    const chatModify = (mod, jid) => {
        const patch = Settings_1.chatModificationToAppPatch(mod, jid);
        return appPatch(patch);
    };
    ws.on('CB:presence', handlePresenceUpdate);
    ws.on('CB:chatstate', handlePresenceUpdate);
    ws.on('CB:ib,,dirty', async (node) => {
        const { attrs } = Internal_1.getBinaryNodeChild(node, 'dirty');
    ...
    ws.on('CB:notification,type:server_sync', (node) => {
        const update = Internal_1.getBinaryNodeChild(node, 'collection');
        if (update) {
