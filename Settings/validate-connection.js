...
const generateLoginNode = (userJid, config) => {
    const { user, device } = Internal_1.jidDecode(userJid);
    const payload = {
        passive: true,
        connectType: 1,
        connectReason: 1,
    ...
    return Proto_1.proto.ClientPayload.encode(payload).finish();
};
...
const generateRegistrationNode = ({ registrationId, signedPreKey, signedIdentityKey }, config) => {
    ...
    const companionProto = Proto_1.proto.CompanionProps.encode(companion).finish();
    ...
    return Proto_1.proto.ClientPayload.encode(registerPayload).finish();
};
exports.generateRegistrationNode = generateRegistrationNode;
const configureSuccessfulPairing = (stanza, { advSecretKey, signedIdentityKey, signalIdentities }) => {
    var _a, _b, _c, _d, _e;
    const [pair] = Internal_1.getAllBinaryNodeChildren(stanza);
    ...
    const { details, hmac } = Proto_1.proto.ADVSignedDeviceIdentityHMAC.decode(deviceIdentity);
    ...
    const accountMsg = Internal_1.Binary.build(new Uint8Array([6, 0]), account.details, signedIdentityKey.public).readByteArray();
    if (!crypto_1.Curve.verify(accountSignatureKey, accountMsg, accountSignature)) {
        throw new boom_1.Boom('Failed to verify account signature');
    }
    const deviceMsg = Internal_1.Binary.build(new Uint8Array([6, 1]), account.details, signedIdentityKey.public, account.accountSignatureKey).readByteArray();
    ...
    const keyIndex = Proto_1.proto.ADVDeviceIdentity.decode(account.details).keyIndex;
    const accountEnc = Proto_1.proto.ADVSignedDeviceIdentity.encode({
        ...account.toJSON(),
