...
const MessageTypeProto = {
    'image': Models_1.WAProto.ImageMessage,
    'video': Models_1.WAProto.VideoMessage,
    'audio': Models_1.WAProto.AudioMessage,
    'sticker': Models_1.WAProto.StickerMessage,
    'document': Models_1.WAProto.DocumentMessage,
};
const ButtonType = Proto_1.proto.ButtonsMessage.ButtonsMessageHeaderType;
const prepareWAMessageMedia = async (message, options) => {
    const logger = options.logger;
    let mediaType;
    for (const key of Mode_1.MEDIA_KEYS) {
        if (key in message) {
            mediaType = key;
        }
    }
    ...
    if (cacheableKey) {
        ...
        if (mediaBuff) {
            const obj = Models_1.WAProto.Message.decode(mediaBuff);
    ...
    delete uploadData.media;
    const obj = Models_1.WAProto.Message.fromObject({
    ...
    if (cacheableKey) {
        ...
        options.mediaCache.set(cacheableKey, Models_1.WAProto.Message.encode(obj).finish());
    }
    return obj;
};
...
const prepareDisappearingMessageSettingContent = (ephemeralExpiration) => {
    ...
    const content = {
        ephemeralMessage: {
            message: {
                protocolMessage: {
                    type: Models_1.WAProto.ProtocolMessage.ProtocolMessageType.EPHEMERAL_SETTING,
                    ephemeralExpiration
                }
            }
        }
    };
    return Models_1.WAProto.Message.fromObject(content);
};
...
const generateForwardMessageContent = (message, forceForward) => {
    ...
    // hacky copy
    content = Proto_1.proto.Message.decode(Proto_1.proto.Message.encode(message.message).finish());
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (!!options.getUrlInfo && message.text.match(Mode_1.URL_REGEX)) {
            try {
        ...
        if (contactLen === 1) {
            m.contactMessage = Models_1.WAProto.ContactMessage.fromObject(message.contacts.contacts[0]);
        }
        else {
            m.contactsArrayMessage = Models_1.WAProto.ContactsArrayMessage.fromObject(message.contacts);
        }
    }
    else if ('location' in message) {
        m.locationMessage = Models_1.WAProto.LocationMessage.fromObject(message.location);
    }
    else if ('delete' in message) {
        m.protocolMessage = {
            key: message.delete,
            type: Models_1.WAProto.ProtocolMessage.ProtocolMessageType.REVOKE
        };
    }
        ...
        const exp = typeof message.disappearingMessagesInChat === 'boolean' ?
            (message.disappearingMessagesInChat ? Mode_1.WA_DEFAULT_EPHEMERAL : 0) :
    ...
    if ('buttons' in message && !!message.buttons) {
        const buttonsMessage = {
            buttons: message.buttons.map(b => ({ ...b, type: Proto_1.proto.Button.ButtonType.RESPONSE }))
        };
        if ('text' in message) {
    ...
    if ('sections' in message && !!message.sections) {
        const listMessage = {
            ...
            listType: Proto_1.proto.ListMessage.ListMessageListType['SINGLE_SELECT']
        };
        m = { listMessage };
    }
    ...
    return Models_1.WAProto.Message.fromObject(m);
};
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    !!(options === null || options === void 0 ? void 0 : options.ephemeralExpiration) &&
        ...
        message[key].contextInfo = {
            ...(message[key].contextInfo || {}),
            expiration: options.ephemeralExpiration || Mode_1.WA_DEFAULT_EPHEMERAL,
    ...
    message = Models_1.WAProto.Message.fromObject(message);
    const messageJSON = {
        key: {
            remoteJid: jid,
            fromMe: true,
        ...
        status: Models_1.WAMessageStatus.PENDING
    };
    return Models_1.WAProto.WebMessageInfo.fromObject(messageJSON);
};
