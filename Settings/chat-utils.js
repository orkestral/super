...
const generateMac = (operation, data, keyId, key) => {
    const getKeyData = () => {
        let r;
        switch (operation) {
            case Proto_1.proto.SyncdMutation.SyncdMutationSyncdOperation.SET:
                r = 0x01;
                break;
            case Proto_1.proto.SyncdMutation.SyncdMutationSyncdOperation.REMOVE:
                r = 0x02;
                break;
        }
...
const makeLtHashGenerator = ({ indexValueMap, hash }) => {
    ...
    return {
        mix: ({ indexMac, valueMac, operation }) => {
            ...
            if (operation === Proto_1.proto.SyncdMutation.SyncdMutationSyncdOperation.REMOVE) {
                if (!prevOp) {
...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        const operation = 'operation' in msgMutation ? msgMutation.operation : Proto_1.proto.SyncdMutation.SyncdMutationSyncdOperation.SET;
...
const extractSyncdPatches = async (result) => {
    const syncNode = Internal_1.getBinaryNodeChild(result, 'sync');
    const collectionNodes = Internal_1.getBinaryNodeChildren(syncNode, 'collection');
    const final = {};
    await Promise.all(collectionNodes.map(async (collectionNode) => {
        const patchesNode = Internal_1.getBinaryNodeChild(collectionNode, 'patches');
        const patches = Internal_1.getBinaryNodeChildren(patchesNode || collectionNode, 'patch');
        const snapshotNode = Internal_1.getBinaryNodeChild(collectionNode, 'snapshot');
        const syncds = [];
...
const chatModificationToAppPatch = (mod, jid) => {
    const OP = Proto_1.proto.SyncdMutation.SyncdMutationSyncdOperation;
