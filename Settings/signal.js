...
const jidToSignalSenderKeyName = (group, user) => {
    return new Signal_1.SenderKeyName(group, exports.jidToSignalProtocolAddress(user)).toString();
};
...
const signalStorage = ({ creds, keys }) => ({
    ...
    loadSenderKey: async (keyId) => {
        ...
        if (key)
            return new Signal_1.SenderKeyRecord(key);
    },
...
const decryptGroupSignalProto = (group, user, msg, auth) => {
    ...
    const cipher = new Signal_1.GroupCipher(exports.signalStorage(auth), senderName);
...
const processSenderKeyMessage = async (authorJid, item, auth) => {
    const builder = new Signal_1.GroupSessionBuilder(exports.signalStorage(auth));
    const senderName = exports.jidToSignalSenderKeyName(item.groupId, authorJid);
    const senderMsg = new Signal_1.SenderKeyDistributionMessage(null, null, null, null, item.axolotlSenderKeyDistributionMessage);
    const { [senderName]: senderKey } = await auth.keys.get('sender-key', [senderName]);
    if (!senderKey) {
        const record = new Signal_1.SenderKeyRecord();
...
const encryptSenderKeyMsgSignalProto = async (group, data, meId, auth) => {
    ...
    const builder = new Signal_1.GroupSessionBuilder(storage);
    const { [senderName]: senderKey } = await auth.keys.get('sender-key', [senderName]);
    if (!senderKey) {
        const record = new Signal_1.SenderKeyRecord();
        await auth.keys.set({ 'sender-key': { [senderName]: record } });
    ...
    const session = new Signal_1.GroupCipher(storage, senderName);
    return {
...
const parseAndInjectE2ESessions = async (node, auth) => {
    const extractKey = (key) => (key ? ({
        keyId: Internal_1.getBinaryNodeChildUInt(key, 'id', 3),
        publicKey: exports.generateSignalPubKey(Internal_1.getBinaryNodeChildBuffer(key, 'value')),
        signature: Internal_1.getBinaryNodeChildBuffer(key, 'signature'),
    }) : undefined);
    const nodes = Internal_1.getBinaryNodeChildren(Internal_1.getBinaryNodeChild(node, 'list'), 'user');
    for (const node of nodes) {
        Internal_1.assertNodeErrorFree(node);
    }
    await Promise.all(nodes.map(async (node) => {
        const signedKey = Internal_1.getBinaryNodeChild(node, 'skey');
        const key = Internal_1.getBinaryNodeChild(node, 'key');
        const identity = Internal_1.getBinaryNodeChildBuffer(node, 'identity');
        const jid = node.attrs.jid;
        const registrationId = Internal_1.getBinaryNodeChildUInt(node, 'registration', 4);
        const device = {
            registrationId,
...
const extractDeviceJids = (result, myJid, excludeZeroDevices) => {
    var _a;
    const { user: myUser, device: myDevice } = Internal_1.jidDecode(myJid);
    const extracted = [];
    for (const node of result.content) {
        const list = (_a = Internal_1.getBinaryNodeChild(node, 'list')) === null || _a === void 0 ? void 0 : _a.content;
        if (list && Array.isArray(list)) {
            for (const item of list) {
                const { user } = Internal_1.jidDecode(item.attrs.jid);
                const devicesNode = Internal_1.getBinaryNodeChild(item, 'devices');
                const deviceListNode = Internal_1.getBinaryNodeChild(devicesNode, 'device-list');
