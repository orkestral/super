...
const makeSocket = ({ waWebSocketUrl, connectTimeoutMs, phoneResponseTimeMs, logger, agent, keepAliveIntervalMs, expectResponseTimeout, }) => {
    // for generating tags
    const referenceDateSeconds = Settings_1.unixTimestampSeconds(new Date());
    ...
    const sendNode = async ({ json, binaryTag, tag, longTag }) => {
        ...
        if (binaryTag) {
            ...
            const binary = Internal_1.encodeBinaryNodeLegacy(json); // encode the JSON to the WhatsApp binary format
            const buff = Settings_1.aesEncrypt(binary, authInfo.encKey); // encrypt it using AES and our encKey
            const sign = Settings_1.hmacSign(buff, authInfo.macKey); // sign the message using HMAC and our macKey
            data = Buffer.concat([
                Buffer.from(tag + ','),
    ...
    const onMessageRecieved = (message) => {
        ...
        if (message[0] === '!' || message[0] === '!'.charCodeAt(0)) {
            ...
            let json;
            try {
                const dec = Settings_1.decodeWAMessage(message, authInfo);
                messageTag = dec[0];
                json = dec[1];
                if (!json)
                    return;
            ...
            anyTriggered = ws.emit(`${Mode_1.DEF_TAG_PREFIX}${messageTag}`, json);
            ...
            Object.keys(l1).forEach(key => {
                anyTriggered = ws.emit(`${Mode_1.DEF_CALLBACK_PREFIX}${l0},${key}:${l1[key]},${l2}`, json) || anyTriggered;
                anyTriggered = ws.emit(`${Mode_1.DEF_CALLBACK_PREFIX}${l0},${key}:${l1[key]}`, json) || anyTriggered;
                anyTriggered = ws.emit(`${Mode_1.DEF_CALLBACK_PREFIX}${l0},${key}`, json) || anyTriggered;
            });
            anyTriggered = ws.emit(`${Mode_1.DEF_CALLBACK_PREFIX}${l0},,${l2}`, json) || anyTriggered;
            anyTriggered = ws.emit(`${Mode_1.DEF_CALLBACK_PREFIX}${l0}`, json) || anyTriggered;
    ...
    const exitQueryIfResponseNotExpected = (tag, cancel) => {
        ...
        const listener = ([, connected]) => {
            if (connected) {
                ...
                ws.off(Mode_1.PHONE_CONNECTION_CB, listener);
            }
        };
        ws.on(Mode_1.PHONE_CONNECTION_CB, listener);
        return () => {
            ws.off(Mode_1.PHONE_CONNECTION_CB, listener);
    ...
    const waitForMessage = (tag, requiresPhoneConnection, timeoutMs) => {
        if (ws.readyState !== ws.OPEN) {
            throw new boom_1.Boom('Connection not open', { statusCode: Models_1.DisconnectReason.connectionClosed });
        }
        let cancelToken = () => { };
        return {
            promise: (async () => {
                ...
                try {
                    const result = await Settings_1.promiseTimeout(timeoutMs, (resolve, reject) => {
                        onRecv = resolve;
                        onErr = err => {
                            reject(err || new boom_1.Boom('Intentional Close', { statusCode: Models_1.DisconnectReason.connectionClosed }));
                        };
    ...
    const waitForSocketOpen = async () => {
        if (ws.readyState === ws.OPEN)
            return;
        if (ws.readyState === ws.CLOSED || ws.readyState === ws.CLOSING) {
            throw new boom_1.Boom('Connection Already Closed', { statusCode: Models_1.DisconnectReason.connectionClosed });
        }
        let onOpen;
        let onClose;
    ...
    ws.on('error', end);
    ws.on('close', () => end(new boom_1.Boom('Connection Terminated', { statusCode: Models_1.DisconnectReason.connectionLost })));
    ws.on(Mode_1.PHONE_CONNECTION_CB, json => {
        if (!json[1]) {
            end(new boom_1.Boom('Connection terminated by phone', { statusCode: Models_1.DisconnectReason.connectionLost }));
    ...
    ws.on('CB:Cmd,type:disconnect', json => {
        ...
        switch (kind) {
            case 'replaced':
                reason = Models_1.DisconnectReason.connectionReplaced;
                break;
            default:
                reason = Models_1.DisconnectReason.connectionLost;
                break;
        }
    ...
    return {
        ...
        setQuery: async (nodes, binaryTag = [Models_1.WAMetric.group, Models_1.WAFlag.ignore], tag) => {
            const json = {
                tag: 'action',
