...
const makeAuthSocket = (config) => {
    ...
    const authInfo = initialAuthInfo || Settings_1.newLegacyAuthCreds();
    const state = {
        legacy: {
    ...
    const logout = async () => {
        ...
        socket === null || socket === void 0 ? void 0 : socket.end(new boom_1.Boom('Logged Out', { statusCode: Models_1.DisconnectReason.loggedOut }));
    };
    const updateEncKeys = () => {
    ...
    const generateKeysForAuth = async (ref, ttl) => {
        curveKeys = Settings_1.Curve.generateKeyPair();
    ...
    const onOpen = async () => {
        ...
        if ((_a = response[1]) === null || _a === void 0 ? void 0 : _a.challenge) {
            const json = Settings_1.computeChallengeResponse(response[1].challenge, authInfo);
        ...
        if (response[1].type === 'upgrade_md_prod') {
            throw new boom_1.Boom('Require multi-device edition', { statusCode: Models_1.DisconnectReason.multideviceMismatch });
        }
        // validate the new connection
        const { user, auth } = Settings_1.validateNewConnection(response[1], authInfo, curveKeys); // validate the connection
