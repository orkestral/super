...
const STATUS_MAP = {
    read: Models_1.WAMessageStatus.READ,
    message: Models_1.WAMessageStatus.DELIVERY_ACK,
    error: Models_1.WAMessageStatus.ERROR
};
const makeMessagesSocket = (config) => {
    ...
    const fetchMessagesFromWA = async (jid, count, cursor) => {
        ...
        const { content } = await query({
            ...
            binaryTag: [Models_1.WAMetric.queryMessages, Models_1.WAFlag.ignore],
            expect200: false,
            requiresPhoneConnection: true
        });
        if (Array.isArray(content)) {
            return content.map(data => Proto_1.proto.WebMessageInfo.decode(data.content));
        }
        return [];
    };
    const updateMediaMessage = async (message) => {
        ...
        const response = await query({
            ...
            binaryTag: [Models_1.WAMetric.queryMedia, Models_1.WAFlag.ignore],
            expect200: true,
            requiresPhoneConnection: true
        });
    ...
    const onMessage = (message, type) => {
        ...
        if (message.message) {
            chatUpdate.conversationTimestamp = +Settings_1.toNumber(message.messageTimestamp);
            // add to count if the message isn't from me & there exists a message
            if (!message.key.fromMe) {
                chatUpdate.unreadCount = 1;
                const participant = Internal_1.jidNormalizedUser(message.participant || jid);
        ...
        if (protocolMessage) {
            switch (protocolMessage.type) {
                case Proto_1.proto.ProtocolMessage.ProtocolMessageType.REVOKE:
                    ...
                    const messageStubType = Models_1.WAMessageStubType.REVOKE;
        ...
        if (message.messageStubType) {
            ...
            switch (message.messageStubType) {
                case Models_1.WAMessageStubType.CHANGE_EPHEMERAL_SETTING:
                    chatUpdate.ephemeralSettingTimestamp = message.messageTimestamp;
                ...
                case Models_1.WAMessageStubType.GROUP_PARTICIPANT_REMOVE:
                    participants = message.messageStubParameters.map(Internal_1.jidNormalizedUser);
                ...
                case Models_1.WAMessageStubType.GROUP_CREATE:
    ...
    const generateUrlInfo = async (text) => {
        const response = await query({
            ...
            binaryTag: [26, Models_1.WAFlag.ignore],
            expect200: true,
            requiresPhoneConnection: false
        });
    ...
    const relayMessage = async (message, { waitForAck } = { waitForAck: true }) => {
            ...
            content: [
                {
                    tag: 'message',
                    attrs: {},
                    content: Proto_1.proto.WebMessageInfo.encode(message).finish()
                }
            ]
        };
        const isMsgToMe = Internal_1.areJidsSameUser(message.key.remoteJid, ((_a = state.legacy.user) === null || _a === void 0 ? void 0 : _a.id) || '');
        const flag = isMsgToMe ? Models_1.WAFlag.acknowledge : Models_1.WAFlag.ignore; // acknowledge when sending message to oneself
        const mID = message.key.id;
        const finalState = isMsgToMe ? Models_1.WAMessageStatus.READ : Models_1.WAMessageStatus.SERVER_ACK;
        message.status = Models_1.WAMessageStatus.PENDING;
        const promise = query({
            json,
            binaryTag: [Models_1.WAMetric.message, flag],
            tag: mID,
            expect200: true,
            requiresPhoneConnection: true
        });
        if (waitForAck) {
            ...
            promise
                ...
                .catch(() => emitUpdate(Models_1.WAMessageStatus.ERROR));
        }
    ...
    const messagesUpdate = (node, isLatest) => {
        const messages = Internal_1.getBinaryNodeMessages(node);
        messages.reverse();
    ...
    socketEvents.on('CB:action,add:relay,message', (node) => {
        const msgs = Internal_1.getBinaryNodeMessages(node);
    ...
    const onMessageStatusUpdate = ({ content }) => {
        if (Array.isArray(content)) {
            ...
            for (const { attrs: json } of content) {
                const key = {
                    remoteJid: Internal_1.jidNormalizedUser(json.jid),
                    id: json.index,
    ...
    const onMessageInfoUpdate = ([, attributes]) => {
        ...
        const keyPartial = {
            remoteJid: Internal_1.jidNormalizedUser(attributes.to),
            fromMe: Internal_1.areJidsSameUser(attributes.from, ((_b = (_a = state.legacy) === null || _a === void 0 ? void 0 : _a.user) === null || _b === void 0 ? void 0 : _b.id) || ''),
        };
        const updates = ids.map(id => ({
            key: { ...keyPartial, id },
            update: {
                [updateKey]: { [Internal_1.jidNormalizedUser(attributes.participant || attributes.to)]: new Date(+attributes.t) }
            }
        }));
        ev.emit('message-info.update', updates);
        // for individual messages
        // it means the message is marked read/delivered
        if (!Internal_1.isJidGroup(keyPartial.remoteJid)) {
            ev.emit('messages.update', ids.map(id => ({
                key: { ...keyPartial, id },
                update: {
                    status: updateKey === 'deliveries' ? Models_1.WAMessageStatus.DELIVERY_ACK : Models_1.WAMessageStatus.READ
                }
            })));
        }
    };
    ...
    return {
        ...
        messageInfo: async (jid, messageID) => {
            const { content } = await query({
                ...
                binaryTag: [Models_1.WAMetric.queryRead, Models_1.WAFlag.ignore],
                expect200: true,
                requiresPhoneConnection: true
            });
            ...
            if (Array.isArray(content)) {
                for (const { tag, content: innerData } of content) {
                    ...
                    const jid = Internal_1.jidNormalizedUser(attrs.jid);
        ...
        downloadMediaMessage: async (message, type = 'buffer') => {
            const downloadMediaMessage = async () => {
                let mContent = Settings_1.extractMessageContent(message.message);
                if (!mContent)
                    throw new boom_1.Boom('No message present', { statusCode: 400, data: message });
                const stream = await Settings_1.decryptMediaMessageBuffer(mContent);
                if (type === 'buffer') {
        ...
        searchMessages: async (txt, inJid, count, page) => {
            ...
            const node = await query({
                ...
                binaryTag: [24, Models_1.WAFlag.ignore],
                expect200: true
            }); // encrypt and send  off
            return {
                last: ((_a = node.attrs) === null || _a === void 0 ? void 0 : _a.last) === 'true',
                messages: Internal_1.getBinaryNodeMessages(node)
            };
        },
        sendMessage: async (jid, content, options = { waitForAck: true }) => {
            ...
            if (typeof content === 'object' &&
                ...
                const msg = await Settings_1.generateWAMessage(jid, content, {
                    logger,
                    userJid: userJid,
