...
const generics_1 = require("../Settings/generics");
...
const makeChatsSocket = (config) => {
    ...
    const sendChatsQuery = (epoch) => (sendNode({
        ...
        binaryTag: [Models_1.WAMetric.queryChat, Models_1.WAFlag.ignore]
    }));
    ...
    const executeChatModification = (node) => {
        ...
        const jid = Internal_1.jidNormalizedUser(attributes === null || attributes === void 0 ? void 0 : attributes.jid);
        switch (updateType) {
            case 'delete':
    ...
    const applyingPresenceUpdate = (update) => {
        const id = Internal_1.jidNormalizedUser(update.id);
        const participant = Internal_1.jidNormalizedUser(update.participant || update.id);
        const presence = {
    ...
    const chatRead = async (fromMessage, count) => {
        ...
        ], [Models_1.WAMetric.read, Models_1.WAFlag.ignore]);
        ...
        try {
            await Promise.all([
                sendNode({
                    ...
                    binaryTag: [Models_1.WAMetric.queryContact, Models_1.WAFlag.ignore]
                }),
                sendNode({
                    json: { tag: 'query', attrs: { type: 'status', epoch: '1' } },
                    binaryTag: [Models_1.WAMetric.queryStatus, Models_1.WAFlag.ignore]
                }),
                sendNode({
                    json: { tag: 'query', attrs: { type: 'quick_reply', epoch: '1' } },
                    binaryTag: [Models_1.WAMetric.queryQuickReply, Models_1.WAFlag.ignore]
                }),
                sendNode({
                    json: { tag: 'query', attrs: { type: 'label', epoch: '1' } },
                    binaryTag: [Models_1.WAMetric.queryLabel, Models_1.WAFlag.ignore]
                }),
                sendNode({
                    json: { tag: 'query', attrs: { type: 'emoji', epoch: '1' } },
                    binaryTag: [Models_1.WAMetric.queryEmoji, Models_1.WAFlag.ignore]
                }),
                sendNode({
                    json: {
                        tag: 'action',
                    ...
                    binaryTag: [Models_1.WAMetric.presence, Models_1.WAFlag.available]
                })
            ]);
    ...
    socketEvents.on('CB:response,type:chat', async ({ content: data }) => {
        ...
        if (Array.isArray(data)) {
            ...
            const chats = data.map(({ attrs }) => {
                const id = Internal_1.jidNormalizedUser(attrs.jid);
                if (attrs.name) {
                    contacts.push({ id, name: attrs.name });
                }
                return {
                    id: Internal_1.jidNormalizedUser(attrs.jid),
    ...
    socketEvents.on('CB:Status,status', json => {
        const id = Internal_1.jidNormalizedUser(json[1].id);
    ...
    socketEvents.on('CB:Cmd,type:picture', async (json) => {
        json = json[1];
        const id = Internal_1.jidNormalizedUser(json.jid);
    ...
    socketEvents.on('CB:action,,user', (json) => {
        if (Array.isArray(json.content)) {
            ...
            if (user.id) {
                user.id = Internal_1.jidNormalizedUser(user.id);
    ...
    return {
        ...
        chatModify: async (modification, jid, chatInfo, timestampNow) => {
            ...
            const response = await setQuery([node], [Models_1.WAMetric.chat, Models_1.WAFlag.ignore]);
            if (config.emitOwnEvents) {
        ...
        sendPresenceUpdate: (type, jid) => (sendNode({
            binaryTag: [Models_1.WAMetric.presence, Models_1.WAFlag[type]],
            json: {
                tag: 'action',
        ...
        async updateProfilePicture(jid, img) {
            ...
            const { eurl } = await this.setQuery([query], [Models_1.WAMetric.picture, 136], tag);
            if (config.emitOwnEvents) {
                if (jid === user.id) {
                    user.imgUrl = eurl;
        ...
        blockUser: async (jid, type = 'add') => {
            ...
            await setQuery([json], [Models_1.WAMetric.block, Models_1.WAFlag.ignore]);
            if (config.emitOwnEvents) {
        ...
        getBusinessProfile: async (jid) => {
            ...
            return {
                ...profile,
                wid: Internal_1.jidNormalizedUser(wid)
            };
        }
    };
};
