...
const makeGroupsSocket = (config) => {
    ...
    const groupQuery = async (type, jid, subject, participants, additionalNodes) => {
        ...
        ], [Models_1.WAMetric.group, 136], tag);
        return result;
    };
    ...
    const groupMetadataFull = async (jid) => {
        ...
        const meta = {
            ...
            owner: Internal_1.jidNormalizedUser(metadata.owner),
            desc: metadata.desc,
            descOwner: metadata.descOwner,
            participants: metadata.participants.map(p => ({
                id: Internal_1.jidNormalizedUser(p.id),
    ...
    const groupMetadataMinimal = async (jid) => {
        const { attrs, content } = await query({
            ...
            binaryTag: [Models_1.WAMetric.group, Models_1.WAFlag.ignore],
            expect200: true
        });
        const participants = [];
        let desc;
    ...
    return {
        ...
        getBroadcastListInfo: async (jid) => {
            ...
            const metadata = {
                ...
                participants: result.recipients.map(({ id }) => ({ id: Internal_1.jidNormalizedUser(id), isAdmin: false, isSuperAdmin: false }))
            };
            return metadata;
        },
